...
const MyCombats: React.FC = () => {
  ...
  function getOpponent(combat: Combat) {
    ...
    if (combat.creator === userId) {
      ...
      if ((combat as any).opponent && typeof (combat as any).opponent === "object" && (combat as any).opponent.name) {
        return (combat as any).opponent.name;
      }
      // Buscar en boxers
      if (Array.isArray((combat as any).boxers)) {
        const other = (combat as any).boxers.find((b: any) =>
          (typeof b === "object" && b._id !== userId && b.name !== userName) ||
          (typeof b === "string" && b !== userId && b !== userName)
        );
        if (other) return typeof other === "object" ? other.name || other._id : other;
      }
      // Fallback
      return combat.opponent;
    } else {
      if ((combat as any).creator && typeof (combat as any).creator === "object" && (combat as any).creator.name) {
  ...
  function getOpponentForFuture(combat: Combat) {
    ...
    if (
      (combat.creator === userId || (combat as any).creator?.name === userName) &&
      (combat.opponent === userId || (combat as any).opponent?.name === userName)
    ) {
    ...
    if (combat.opponent === userId || (combat as any).opponent?.name === userName) {
      return (combat as any).creator?.name || combat.creator || "-";
    }
    // Si el creador coincide con el usuario logueado, muestra el oponente
    if (combat.creator === userId || (combat as any).creator?.name === userName) {
  ...
  const handleInvitationResponse = async (combatId: string, status: "accepted" | "rejected") => {
    try {
  ...
  if (loading) return <div className="my-combats-loading">{t("loadingCombats")}</div>;
  ...
  return (
    <div className="my-combats-container">
        ...
        <ul className="my-combats-list">
          {futureCombats.map((c) => (
            <li key={c._id} className="my-combats-item">
              <div>
                <span className="my-combats-label">{t("date")}:</span> {formatDate(String(c.date))}
                <span className="my-combats-label"> | {t("time")}:</span> {formatTime(c.time)}
              </div>
              <div>
                <span className="my-combats-label">{t("gym")}:</span> {(c as any).gym?.name || (c as any).gym || "-"}
                {getGymAddress(c) && <span className="my-combats-gym-address"> ({getGymAddress(c)})</span>}
              </div>
              <div>
                <span className="my-combats-label">{t("opponent")}:</span> {getOpponentForFuture(c)}
              </div>
            </li>
          ))}
        </ul>
      ...
      {receivedInvitations.length > 0 ? (
        <ul className="my-combats-list">
          {receivedInvitations.map((c) => (
            <li key={c._id} className="my-combats-item">
              <div>
                <span className="my-combats-label">{t("fromCreator")}:</span> {(c as any).creator?.name || c.creator || "-"}
                <span className="my-combats-label"> | {t("gym")}:</span> {(c as any).gym?.name || (c as any).gym || "-"}
                {getGymAddress(c) && <span className="my-combats-gym-address"> ({getGymAddress(c)})</span>}
              </div>
              <div className="my-combats-actions">
                <button className="my-combats-accept" onClick={() => handleInvitationResponse(c._id!, "accepted")}>{t("accept")}</button>
                <button className="my-combats-decline" onClick={() => handleInvitationResponse(c._id!, "rejected")}>{t("decline")}</button>
              </div>
            </li>
          ))}
        </ul>
      ...
      <h2 className="my-combats-section-title">⏳ {t("pendingCombatsSentTitle")}</h2>
      {sentInvitations.length > 0 ? (
        <ul className="my-combats-list">
          {sentInvitations.map((c) => (
            <li key={c._id} className="my-combats-item">
              <div>
                <span className="my-combats-label">{t("toOpponent")}:</span> {getOpponent(c)}
                <span className="my-combats-label"> | {t("date")}:</span> {formatDate(String(c.date))}
                <span className="my-combats-label"> | {t("status")}:</span> {t("waitingForResponse") || "Esperando una respuesta"}
              </div>
            </li>
          ))}
        </ul>
